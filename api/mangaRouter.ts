...
export const mangaRouter = createRouter({
  ...
  similar: publicQuery
    ...
    .query(async ({ input }) => {
      ...
      const candidates = await db.query.manga.findMany({
      ...
      return candidates
        .map((c) => ({
          manga: c,
          overlap: (c.genres ?? []).filter((g) => baseGenres.has(g)).length,
        }))
