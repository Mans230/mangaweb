...
export class AzoraFlyScraper extends BaseScraper {
  ...
  async getSeries(urlOrSlug: string): Promise<SeriesInfo> {
    const slug = String(urlOrSlug).includes("/series/")
      ...
      : String(urlOrSlug);
    // لا يوجد endpoint مباشر للسلسلة — نبحث بالـ slug/العنوان ونطابق.
    // عناوين طويلة قد لا تطابق بحث المصدر — قلّص الكلمات تدريجياً حتى نجد الـ slug.
  ...
  async getPages(chapterUrl: string): Promise<string[]> {
    ...
    $("img").each((_, img) => {
      ...
      // صور الفصول مستضافة على storage.azorafly.com
      if (/storage\.azorafly\.com/i.test(abs) && !/logo|icon|avatar|banner/i.test(abs)) {
        pages.push(abs);
      }
    });
    return pages;
